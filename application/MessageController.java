...
public class MessageController implements Initializable 
{
    ...
    {
    	HalalPairing.findMatch(HalalPairing.getCurrentUser().getCNIC());
    	HashMap<String, Double> matches = HalalPairing.getAllMatches(HalalPairing.getCurrentUser().getCNIC());
    	
        try 
        {
           for (Entry<String, Double> entry : matches.entrySet()) 
           {
                FXMLLoader fxmlloader = new FXMLLoader(getClass().getResource("MessageProfile.fxml"));
                AnchorPane anchorPane= fxmlloader.load();
                MessageProfile postController = fxmlloader.getController();
                postController.setData(HalalPairing.getUser(entry.getKey()),entry.getKey());
